...
@router.get('/prediction', response_model=schemas.AllPrediction, status_code=status.HTTP_200_OK)
async def get_prediction_history(request: Request, db: AsyncSession = Depends(get_async_db)):
    try:
        ...
        total_count_result = await db.execute(count_stmt, {"user_id": user_id})
        ...
        predictions = result.mappings().all()  # returns list of dict-like rows
...
#     MODEL_PATH = Path(__file__).parent / "plant_disease_model_1_latest.pt"
...
#         print(confidence)
#         print("result_index before",result_index)
#         result_index = max(0, result_index - 1)
#         print("result_index after",result_index)
