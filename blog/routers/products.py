...
@router.get('/user-products/{user_id}', status_code=status.HTTP_200_OK, response_model=List[schemas.Products])
async def get_user_products(user_id: str, db: AsyncSession = Depends(get_async_db)):
    try:
        result = await db.execute(_get_product_history, {"user_id": user_id})
